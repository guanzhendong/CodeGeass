...
#define kMaxDistance        43 // 53，官振东修改，QQ没有这么高
...
- (void)endRefreshingWithStyle:(ODEndRefreshStyle)style {
    ...
    if (_refreshing) {
        ...
        // 自定义label
        UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake((self.bounds.size.width - 60) / 2 + 10, kTotalViewHeight - 31, 60, 20)];
        label.text = style == ODEndRefreshStyleSuccess ? @"刷新成功" : @"刷新失败";
        label.textAlignment = NSTextAlignmentCenter;
        label.textColor = [UIColor darkGrayColor];
        label.font = [UIFont systemFontOfSize:13];
        [self addSubview:label];
        UIImageView *face = [[UIImageView alloc] initWithFrame:CGRectMake(self.bounds.size.width/2 - 40, kTotalViewHeight - 30, 18, 18)];
        face.image = [UIImage imageNamed:style == ODEndRefreshStyleSuccess ? @"refresh_face_success" : @"refresh_face_failure"];
